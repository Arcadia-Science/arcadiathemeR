...
To use the custom fonts you need to install [Atkinson Hyperlegible Next](https://fonts.google.com/specimen/Atkinson+Hyperlegible+Next) and [Atkinson Hyperlegible Mono](https://fonts.google.com/specimen/Atkinson+Hyperlegible+Mono) from Google Fonts. Click "Get font," "Download all," then install the `.ttf` files from the `static/` folder inside each zip (double-click to install via FontBook on macOS, or copy them to `~/Library/Fonts/`). Note: the `extrafont` package requires the static TTF files -- do not use the variable font files in the zip root. This should only need to be performed once even if the package is updated over time. If the custom fonts are not available then the system `sans` font will be used as a fallback. These steps and functionality have only been confirmed to work on macOS.
